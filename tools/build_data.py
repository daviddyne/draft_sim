...
def main():
    ...
    # Newest first, so a run that is stopped early still covers current sets
    codes.sort(key=lambda c: names.get(c.upper(), ('', ''))[1], reverse=True)
    ...
    for code in codes:
        ...
        # No early exit here, the pair files below are built even when the card
        ...
        todo = [p for p in PAIRS
                if refresh_all or code in active
                or not os.path.exists(f'{OUT}/{code}_PremierDraft_{p}.json')]
        if todo:
            # Five at a time, ten sequential requests per set made this crawl
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as pool:
                fetched = list(pool.map(
                    lambda p: (p, get(f'https://www.17lands.com/api/card_data?expansion={code}'
                                      f'&event_type=PremierDraft&time_period=ALL_TIME&colors={p}')),
                    todo))
            for pair, raw in fetched:
                rows = (raw.get('data') if isinstance(raw, dict) else raw) or []
                if not rows:
                    continue
                slim = [{'name': r.get('name', ''), 'gihwr': r.get('ever_drawn_win_rate')}
                        for r in rows if r.get('name')]
                with open(f'{OUT}/{code}_PremierDraft_{pair}.json', 'w') as f:
                    json.dump(slim, f)
            print(f'  {code}: {len(fetched)} pair files')
