...
class SeventeenLandsService {
  ...
  Future<List<CardRating>> fetchRatings(
    ...
    for (final event in _eventsToTry(eventType)) {
      final cached = await cache.load(setCode, event);
      if (cached != null && cached.isNotEmpty) return cached;
      try {
        final cards = await _fetchOnline(setCode, event, timePeriod);
        if (cards.isEmpty) continue;
        if (save) await cache.save(setCode, event, cards);
        return cards;
      } catch (e) {
        firstError ??= e;
      }
    }
    throw Exception('No data for $setCode in any draft format ($firstError)');
  }

  // The chosen format first, then the rest as fallbacks
  static List<String> _eventsToTry(String eventType) {
    const all = ['PremierDraft', 'QuickDraft', 'TradDraft'];
    return [eventType, ...all.where((e) => e != eventType)];
  }
